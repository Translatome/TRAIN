...
unexpected patterns, such as batch effects, that should be adjusted. Statisical 
analysis can be performed by choosing between the limma-voom or DESeq2 tool. 
If the samples are too scattered, use the limma-voom tool for batch effect 
adjustment.
...
pander(cat(
  "<figure><img src=", mds_raw, " alt='MDS_raw' style='width:80%'> 
  <figcaption>Fig.1 - Multdimensional scaling plot (MDS) on raw data.
  </figcaption></figure>" 
))
```
...
if (var_tool=="limma"){
  pander(cat(
    ...
    combination of Fraction, Treatment and Time</li>", "<li> Replica </li> </ul> For the construction of the design, use the documentation of [limma](https://bioconductor.org/packages/release/bioc/html/limma.html)."
  ))
}else{
  pander(cat(
    "The design of the model is defined with DESeq2, it corresponds to ",
    "~combine with:" ,"<br>","<ul>","<li> combine which is the combination 
    of Fraction, Treatment and Time </li> </ul> For the construction of the design, use the documentation of [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html)."
  ))
}
...
fraction (cytoplasmic/total vs. polysomal) and time. Double comparisons were 
then performed to categorise the results between transcription and translation.
...
for (li in 1:(length(metacontrast$fraction)/2)) {
  ## Construct the contrast table for each contrast provided 
...
according to the combination of fraction, treatment and time. 
Replicates are highlighted.

```{r plot_norm_data, echo=FALSE, warning=FALSE, message=FALSE}
# Display multi-dimensional scaling plot or PCA of standardised data 
# according to the chosen tool (limma or deseq2)
var_tool <- snakemake@params[["toolstat"]]
if (var_tool=="limma"){
  mds_norm <- paste0(
    "../", snakemake@input[["diff_genes"]], 
    "/plotMDS_norm_data_kinetic.png"
  )
  voom_trend <- paste0(
    "../", snakemake@input[["diff_genes"]], 
    "/mean-variance_trend_kinetic.png"
  )
  screenHist_lim <- paste0(
    "../", snakemake@input[["diff_genes"]], 
    "/ScreeHist_plot.png"
  )
  pander(cat(
    ...
    limma package.</figcaption></figure>", "<figure><img src=", screenHist_lim ,
    " alt='screenHist_lim' style='width:80%'> <figcaption>Fig.3 - Scree-plot for 
    the design described in the 'Design of the model' section for the limma-voom 
    package. </figcaption></figure>", " &nbsp;The scree plot of the captured 
    variability shows how many dimensions are needed to try to reach the 
    threshold of 80% of the variance","<figure><img src=", voom_trend ,
    " alt='voom-trend' style='width:80%'> <figcaption>Fig.4 - The voom trend 
  ...
  pander(cat(
    "<figure><img src=", pca_norm," alt='PCA_deseq'> <figcaption>Fig.2 - 
    Principal component analysis (PCA) performed on normalized data based on 
    the design described in the 'Design of the model' section for the DESeq2 
    package. </figcaption></figure>", "<figure><img src=", dispersion, 
    " alt='dispersion' style='width:45%'><img src=", screenHist,
    " alt='screenHist' style='width:45%'><figcaption>Fig.3 & 4 - Dispersion 
    plot for normalized data and scree-plot for the design described in the 
    'Design of the model' section for the DESeq2 package. </figcaption>
    </figure>", " &nbsp;The dispersion of the data is plotted to see how well 
    it fits the model (the red line). The data used in the analysis are shown 
    in blue. The scree plot of the captured variability shows how many 
...
if (var_tool=="limma"){
  pander(
    "The filtering applicated with limma, keeps genes that have CPM >= 
    CPM.cutoff in MinSampleSize samples, where : <br><ul><li>CPM.cutoff = 
    min.count/median(lib.size)*1e6</li> <li>and MinSampleSize is the smallest 
    group sample size or, more generally, the minimum inverse leverage computed 
    from the design matrix </li></ul><br>(See the filterByExpr() function of 
  ...
  if(snakemake@params[["filtering_method"]] =="edger"){
    pander(
      "The filtering choiced with deseq2, keeps genes that have CPM >= 
      CPM.cutoff in MinSampleSize samples, where : <br><ul><li>CPM.cutoff = 
      min.count/median(lib.size)*1e6</li> <li>and MinSampleSize is the smallest 
      group sample size or, more generally, the minimum inverse leverage 
      computed from the design matrix </li></ul><br>(See the filterByExpr() 
      function of 
      [EdgeR](https://bioconductor.org/packages/release/bioc/html/edgeR.html) 
      package documentation  for more information)."
    )
    ...
    pander(
      "The 'deseq' filtering is performed on the counts that have had an initial
      normalisation on the size of the libraries. If the sum of the counts for 
      a gene is less than 10, the gene is eliminated from the count table (See 
      the 
    ...
    pander(
      "The 'default' filtering is performed on the counts that have had an 
      initial normalisation on the size of the libraries. If the sum of the 
      counts for a gene is less than 1, the gene is eliminated from the count 
      table (See the 
      [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html) 
      package documentation for more information)."
    )
...
present in the experimental data (list of genes obtained after data filtering) 
and in the selected reference genome. 
...
### Number of all genes
tot_all_gene <- nrow(biotype_genome)  
### Remove biotype IG 
filtred_IG <-  subset(
  biotype_genome, 
...
tot_RNA_without_lnc <-  (nrow(
  filtred_pseudo[grepl(
    "RNA", 
    filtred_pseudo$Biotype
  ),]
)) - tot_lncRNA
...
propoall <- data.frame(
  Type_Gene=c(
    ...
    "*pseudogene", "*RNA", 
    "strange", "total"
  ), ### Choose strange for unknown due to alphabetical order 
  Number=c(
...
pp <- ggplot(data = propoall, aes(
  ...
  fill=Type_Gene, text = paste(
    ...
    "<br>Number:", Number,
    "<br>Proportion:", round(Number/tot_all_gene*100, digits = 2), "%"
  )
)) +
...
# selection of one categorization file
...
tot_RNA_without_lnc_exp <- (nrow(
  filtred_pseudo_exp[grepl(
    "RNA", 
    filtred_pseudo_exp$Biotype
  ),]
)) - tot_lncRNA_exp 
...
colnames(comp_simp) <- c(
  "Comparison", "Tot_present_genes", 
  "Tot_filtered_genes", "Sig_genes_tot", 
  "Sig_genes_UP", "Sig_genes_DN"
)
comp_simp %>%
...
The exploration of data to determine the proportion of genes involved in 
translation and/or transcription is illustrated in the following graphs :
...
recap_cat <- data.frame(
  "Not_significant"="", "Total_sig_genes"="",
  "Both_mRNA_UP"="", "Both_mRNA_DOWN"="", 
...
for (index in 1:length(liste_csv)){
  ...
  comp_time  <- sapply(split_name, tail, 2)
  comp <- comp_time[1]
  ## Recovery the time for each comparison
  timing <- comp_time[2]
  timing <- gsub(".tsv", "",timing)
  
  ## Import formatted summary file 
  tablesum <- importsummary(liste_csv[index])
  
  ## Increment the base table with each value from the summary files 
  ## for each comparison
  recap_cat[index,] <- tablesum
  list_comp <- append(list_comp,comp)
  list_time <- append(list_time, timing)
}

recap_cat$Time       <- list_time
recap_cat$Comparison <- list_comp

# Structuring the final table
## Recovery the list of comparison
uniq_comp <- unique(recap_cat$Comparison)

## Initialization of empty table
recap_cat2 <- data.frame(
  "Not_significant"="", "Total_sig_genes"="",
  "Both_mRNA_UP"="", "Both_mRNA_DOWN"="", 
  "Transcription_UP"="", "Transcription_DOWN"="", 
  "Translation_UP"="", "Translation_DOWN"="",
...
## Alternative concatenation of a comparison line with the values corresponding  
## to the comparison and the associated time
for  (i in uniq_comp){
  ### Recovery of values for unique comparison
  sub <- subset(recap_cat, Comparison==i) 
  ...
  recap_cat2 <- rbind(recap_cat2, name_comp)
  ### Concatenation table with table of values
  recap_cat2 <- rbind(recap_cat2, sub)
}
## Remove the first line (empty)
recap_cat2 <- recap_cat2[-1,] 
index_name_comp <- list() #initialization of index for print line with background blue

## Index for print line with background blue and indent line
for (j in 1:nrow(recap_cat2)){
  if (recap_cat2$Comparison[j] == ""){
    index_name_comp <- append(index_name_comp, j)
  }else{
  }
nbline <- 1:nrow(recap_cat2)
index_indent_line <- nbline[-as.numeric(index_name_comp)]

} 
## Clean table
recap_cat2  <- recap_cat2[,4:ncol(recap_cat2)-1]
...
final_table %>%
  ...
  row_spec(
    as.numeric(index_name_comp), 
    bold = T, 
    color = "black", 
    background = "#6CA4C8"
  ) %>%
  add_indent(index_indent_line)
...
if (snakemake@params[["toolenrich"]]=="gprofiler"){
  pander(
    "gProfiler2 performs a functional enrichment analysis on a list of 
    significant input genes.  It compares the genes to known sources of 
    functional information and detects statistically significant enriched terms. 
    The size of the statistical domain N describes the total number of genes 
    used for random selection and is one of the four parameters of the 
    hypergeometric probability function of statistical significance used in 
    gProfiler. Here, we use genes from the reference genome. <br> For the 
    significant genes in each category, in each comparison, we perform an 
    enrichment analysis for the sources of interest. The table below shows 
    the number of enrichment terms found for each source per comparison and 
    category."
  )
} else {
...
if (snakemake@params[["toolenrich"]]=="gprofiler"){
  ...
  pander(cat(
    ...
    category. The histograms below illustrates the number of terms present at 
    each depth of the hierarchy (for BP, MF and CC) for each category for a 
    given comparison. Another variant of the histogram is also available, 
    representing the hierarchy levels. All histograms are available in 
