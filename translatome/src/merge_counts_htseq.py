...
"""
Merge counts obtained from HTSeq counts
=======================================
...
def get_file_df_gff(path):
    """
    Get HTSeq-counts file and prepare it for use
        Counts were performed using a GFF annotation file
    """
    name = get_id(path)
    test = pd.read_csv(path, sep='\t', names=['Geneid', 'Counts'])
...
def get_file_df_gtf(path):
    ...
    """
    name = get_id(path)
    test = pd.read_csv(path, sep='\t', names=['Geneid', 'GeneName', 'Counts'], index_col=['Geneid']) #, index_col='Num'
    test.drop(test.tail(5).index,inplace=True)
    test[name] = test['Counts']
    return test[name]
...
if __name__ == '__main__':
    ...
    INPUTPATH = snakemake.params["inputdir"]
    ...
    if len(dfWTNA) > 2:
        ...
        raise ValueError(f"""Warning: all counts are zero, check params of HTSeq Count""")
