...
#'    The goal is to indicate in the first three columns the fraction, the
#'    treatment and time that we want as a reference for the comparison.
#'    This one will be compared to the same fraction but with a different
#'    treatment at a same time. The fractions and times may differ from one row to another, but
...
REPLICAT          <- snakemake@params[["replicat"]]
...
png(
  file = paste(PATH_OUTPUT, "/plotMDS_norm_data_kinetic.png", sep = "/"),
  width = 900,
  height = 650
)
plot(fig)
...
pdf(file = paste(PATH_OUTPUT, "/plotMDS_norm_data_kinetic.pdf", sep = "/"))
plot(fig)
dev.off()
...
# Voom
png(
  file = paste(PATH_OUTPUT, "mean-variance_trend_kinetic.png", sep = "/"),
  width = 900,
  height = 650
)
...
for (l in 1:length(metacontrast$fraction)) {
  ## Construction of the contrast matrix for each contrast 
  ## provided in the metacontrast file
  ...
  write.table(
    ...
    file = paste(PATH_OUTPUT, "/", 
                 ...
                 metacontrast$treatment1[l], "_",
                 metacontrast$time[l],"_vs_",
                 metacontrast$fraction_bis[l], "_",
                 metacontrast$treatment2[l], "_",
                 metacontrast$time_bis[l],".tsv",
                 sep = ""
    ), 
    sep = "\t",
    dec = ".",
